...
g_UnitPlopper =
{
	...
	function(plot)
		if (g_PlopperSettings.Player ~= -1 and g_UnitPlopper.UnitType ~= -1) then
			local player = Players[g_PlopperSettings.Player] or Players[Game.GetActivePlayer()];
			if (player ~= nil) then
				local unit = player:InitUnit(g_UnitPlopper.UnitType, plot:GetX(), plot:GetY());
				if (g_UnitPlopper.Embarked) then
					unit:Embark();
				end
				if g_UnitPlopper.Experience and g_UnitPlopper.Experience > 0 then
					unit:ChangeExperience(g_UnitPlopper.Experience);
					if g_UnitPlopper.Experience >= 10 then
...
g_RoutePlopper =
{
	...
	Plop =
	function(plot)
		if (g_RoutePlopper.RouteType ~= -1) then
			plot:SetRouteType(g_RoutePlopper.RouteType);
		end
	end,
...
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_AIRSTRIKE][MouseEvents.LButtonUp] = 
function( wParam, lParam )
	local plot = Map.GetPlot( UI.GetMouseOverHex() );
	local plotX = plot:GetX();
	local plotY = plot:GetY();
...
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_RANGE_ATTACK][MouseEvents.LButtonUp] = 
function( wParam, lParam )
	local plot = Map.GetPlot( UI.GetMouseOverHex() );
	local plotX = plot:GetX();
	local plotY = plot:GetY();
...
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_CITY_RANGE_ATTACK][MouseEvents.LButtonUp] = 
function( wParam, lParam )
	local plot = Map.GetPlot( UI.GetMouseOverHex() );
	local plotX = plot:GetX();
	local plotY = plot:GetY();
