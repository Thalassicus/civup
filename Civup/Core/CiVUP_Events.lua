...
function DestroyCourthouseInCapturedCity(city, player, isForced)
	city:SetNumRealBuilding(player:GetUniqueBuildingID("BUILDINGCLASS_COURTHOUSE"), 0)
end
...
if UI:IsLoadedGame() then
	log:Warn("%-10s seconds loading HasTerrainCulture", Game.Round(os.clock() - startClockTime, 8))
end
		...
		if (policyID == testPolicyInfo.ID) or player:HasPolicy(testPolicyInfo.ID) then
			...
			if numCities == -1 then
				for city in player:Cities() do
					...
					if not MapModData.VEM.FreeFlavorBuilding[flavorType][cityID] then
						PlaceBuildingOfFlavor(city, flavorType)
					end
				end
			elseif (MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] < numCities) then
				...
				for i = 1, maxCityID do
					...
					if player:IsHuman() then
						log:Debug("cityID=%s freeBuilding=%s", cityID, MapModData.VEM.FreeFlavorBuilding[flavorType][cityID])
					end
...
if not MapModData.VEM.FreeFlavorBuilding then
	...
	MapModData.VEM.PlayerFreeBuildings = {}
	startClockTime = os.clock()
	for row in GameInfo.Flavors() do
		local flavorType = row.Type
		MapModData.VEM.FreeFlavorBuilding[flavorType] = {}
		MapModData.VEM.PlayerFreeBuildings[flavorType] = {}
		for playerID,player in pairs(Players) do
			MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] = 0
			if UI:IsLoadedGame() then
				for city in player:Cities() do
					local cityID = City_GetID(city)				
					local buildingID = LoadValue("MapModData.VEM.FreeFlavorBuilding[%s][%s]", flavorType, cityID)
					----log:Trace("Loading %15s MapModData.VEM.FreeFlavorBuilding %s = %s", city:GetName(), flavorType, buildingID)
					if buildingID then
						MapModData.VEM.FreeFlavorBuilding[flavorType][cityID] = buildingID
						MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] = MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] + 1
					end
				end
			end
		end
	...
	if UI:IsLoadedGame() then
		log:Warn("%-10s seconds loading FreeFlavorBuilding", Game.Round(os.clock() - startClockTime, 8))
	end
end
...
function PlaceBuildingOfFlavor(city, flavorType)
	...
	if buildingID ~= -1 then
		...
		local cityID = City_GetID(city)
		SaveValue(buildingID, "MapModData.VEM.FreeFlavorBuilding[%s][%s]", flavorType, cityID)
		MapModData.VEM.FreeFlavorBuilding[flavorType][cityID] = buildingID
		MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] = MapModData.VEM.PlayerFreeBuildings[flavorType][playerID] + 1
		if player:IsHuman() then
...
function UpdatePromotions(unit, isUpgrading)
	...
	for swapInfo in GameInfo.UnitPromotions_Equivilancy() do
		...
		for category, oldPromoType in pairs(swapInfo) do
			if oldPromoType == "PROMOTION_SIEGE" and (unitInfo.CombatClass == "_UNITCOMBAT_SIEGE" or unitInfo.CombatClass == "UNITCOMBAT_SIEGE") then
				-- do not replace
	...
	if exp > 0 then
		for unit in player:Units() do
			if unit and not unit:IsDead() and unit:IsGarrisoned() then
				unitXP = (MapModData.VEM.UnitXP[playerID][unit:GetID()] or 0) + exp
				if unitXP >= 1 then
					unit:ChangeExperience(math.floor(unitXP))
					unitXP = unitXP - math.floor(unitXP)
				end
				MapModData.VEM.UnitXP[playerID][unit:GetID()] = unitXP
				SaveValue(unitXP, "MapModData.VEM.UnitXP[%s][%s]", playerID, unit:GetID())
