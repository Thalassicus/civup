    ...
    if (not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
 		for tech in GameInfo.Technologies() do
 			...
 			if player:CanResearch( techID ) then
				AddTechButton( tech, player:GetYieldTurns(YieldTypes.YIELD_SCIENCE,  techID), iDiscover);
			end
		end
	end
...
function AddTechButton( tech, turnsLeft, iDiscover)
	...
	thisTechButtonInstance.FreeTechName:SetText( techName );
  	...
  	local researchTurnsLeft = math.ceil(player:GetYieldTurns(YieldTypes.YIELD_SCIENCE, tech.ID));
	...
	if player:GetCurrentResearch() == tech.ID then -- the player is currently researching this one
		...
		local currentResearchProgress = teamTechs:GetResearchProgress(techID) + player:GetOverflowResearch();
		local researchNeeded = player:GetYieldNeeded(YieldTypes.YIELD_SCIENCE);
		local researchPerTurn = player:GetYieldRate(YieldTypes.YIELD_SCIENCE);
