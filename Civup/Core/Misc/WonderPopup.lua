...
function OnPopup( popupInfo )
	...
	local strGameInfo = GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, false, bExcludeWritten);
    
    if( strGameInfo ~= nil ) then
        Controls.Stats:SetText( Locale.ConvertTextKey( strGameInfo ) );
    	Controls.Stats:SetHide( false );
		Controls.Stats:SetSizeY(Controls.Stats:GetSizeY() + 6)
	...
	--[[
	local numLines = 1
	for row in string.gmatch("strGameInfo", "[NEWLINE]") do
		numLines = numLines + 1
	end
	--]]
