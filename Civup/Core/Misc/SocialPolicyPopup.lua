			...
			if not player:IsPolicyBranchUnlocked( i ) then
				...
				if (not player:CanUnlockPolicyBranch(i)) then
					...
					-- Don't have enough Culture Yet
						...
						strToolTip = strToolTip .. " " .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_CULTURE", player:GetYieldNeeded(YieldTypes.YIELD_CULTURE));
				...
				-- Can adopt this branch right now
					...
					strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_UNLOCK_SPEND", player:GetYieldNeeded(YieldTypes.YIELD_CULTURE));
