...
function RecordYieldInfo( x, y, index, plot )
    ...
    local iCulture    = plot:GetCulture();-- + math.random( 0, 13 );
    
    -- early out of allocations if there is no yield for this tile
    if( iFood == 0 and iProduction == 0 and iGold == 0 and iScience == 0 and iCulture == 0) then
        return;
    end
