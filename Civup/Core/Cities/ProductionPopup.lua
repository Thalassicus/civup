 	...
 	for k,v in ipairs(productionList.Units) do
		...
		--print(v.Name.." "..v.Priority.." "..v.TechCost);
 		if g_IsProductionMode then
			...
			if City_CanPurchase(city, YieldTypes.YIELD_GOLD, GameInfo.Units, unitID, false, false, -1, -1) then
 				local isDisabled = false;
     			-- test w/o visible (ie can train right now)
	     	
    			if (not City_CanPurchase(city, YieldTypes.YIELD_GOLD, GameInfo.Units, unitID, true, true, -1, -1)) then
    				isDisabled = true;
				end
			...
			if City_CanPurchase(city, YieldTypes.YIELD_FAITH, GameInfo.Units, unitID, false, false, -1, -1) then
 				local isDisabled = false;
     			-- test w/o visible (ie can train right now)
	     	
    			if (not City_CanPurchase(city, YieldTypes.YIELD_FAITH, GameInfo.Units, unitID, true, true, -1, -1)) then
    				isDisabled = true;
				end
 	...
 	for k,v in ipairs(productionList.Projects) do
		...
		local project = GameInfo.Projects[projectID];
 	 	if g_IsProductionMode then
			...
			if City_CanPurchase(city, YieldTypes.YIELD_GOLD, GameInfo.Projects, projectID, false, false, -1, -1) then
		...
		if prereq.Info then
 		...
 		if g_IsProductionMode then
			...
			if City_CanBuild(city, buildingID, 0, 1 ) then
				...
				AddProductionButton( YieldTypes.YIELD_PRODUCTION, buildingID, YieldTypes.NO_YIELD, building.Description, OrderTypes.ORDER_CONSTRUCT, strTurnsLeft, col, isDisabled, prereq );
				if col == 2 then
			...
			if City_CanPurchase(city, YieldTypes.YIELD_GOLD, GameInfo.Buildings, buildingID, false, false, -1, -1) then
				local col = 2;
				local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
				if thisBuildingClass.MaxGlobalInstances > 0 or thisBuildingClass.MaxPlayerInstances > 0 or thisBuildingClass.MaxTeamInstances > 0 then
					col = 3;
 				...
 				local isDisabled = false;
    			if not City_CanPurchase(city, YieldTypes.YIELD_GOLD, GameInfo.Buildings, buildingID, true, true, -1, -1) then
 					isDisabled = true;
 				end
			...
			elseif City_CanPurchase(city, YieldTypes.YIELD_FAITH, GameInfo.Buildings, buildingID, false, false, -1, -1) then
				local col = 2;
				local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
				if thisBuildingClass.MaxGlobalInstances > 0 or thisBuildingClass.MaxPlayerInstances > 0 or thisBuildingClass.MaxTeamInstances > 0 then
					col = 3;
 				...
 				local isDisabled = false;
    			if not City_CanPurchase(city, YieldTypes.YIELD_FAITH, GameInfo.Buildings, buildingID, true, true, -1, -1) then
 					isDisabled = true;
 				end
...
----------------------------------------------------------------        
function AddProductionButton( yieldID, id, description, orderType, turnsLeft, column, isDisabled, prereq )	
	local controlTable;
		...
		if (isDisabled) then
			if (g_IsProductionMode) then
				...
				if (ePurchaseYield == YieldTypes.YIELD_GOLD) then
					strDisabledInfo = city:GetPurchaseUnitTooltip(id);
				else
					strDisabledInfo = city:GetFaithPurchaseUnitTooltip(id);
				end
				if (strDisabledInfo ~= nil and strDisabledInfo ~= "") then
					strToolTip = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. strDisabledInfo .. "[ENDCOLOR]";
				end
	...
	elseif column == 2 or column == 3 then -- we are a building, wonder, project, or process
		...
		if orderType == OrderTypes.ORDER_MAINTAIN then
			...
			if (bBuilding) then
				...
				strToolTip = GetHelpTextForBuilding(id, bExcludeName, bExcludeHeader, false, city);
			else
				local bIncludeRequirementsInfo = false;
			...
			if (isDisabled) then
				if (g_IsProductionMode) then
					...
					local strDisabledInfo;
					if (ePurchaseYield == YieldTypes.YIELD_GOLD) then
						strDisabledInfo = city:GetPurchaseBuildingTooltip(id);
					else
						strDisabledInfo = city:GetFaithPurchaseBuildingTooltip(id);
					end
					if (strDisabledInfo ~= nil and strDisabledInfo ~= "") then
						strToolTip = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. strDisabledInfo .. "[ENDCOLOR]";
					end
