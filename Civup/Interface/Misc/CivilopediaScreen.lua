...
CivilopediaCategory[CategoryWonders].PopulateList = function()
	...
	for building in GameInfo.Buildings{IsVisible = true} do	
		-- exclude wonders etc.				
...
CivilopediaCategory[CategoryTech].SelectArticle = function( techID, shouldAddToList )
	...
	if techID ~= -1 then
		...
		local cost = Players[Game.GetActivePlayer()]:GetResearchCost(thisTech.ID);
		if(Game ~= nil) then
		...
		if thisTech.AllowEmbassiesTradingAllowed then
			if numAbilities > 0 then
				abilitiesString = abilitiesString .. "[NEWLINE]";
			end
	...
	if unitID ~= -1 then
		...
		for row in GameInfo.Civilization_UnitClassOverrides( condition ) do
			if row.CivilizationType ~= "CIVILIZATION_BARBARIAN" and row.CivilizationType ~= "CIVILIZATION_MINOR" then
				local otherCondition = "Type = '" .. row.UnitClassType .. "'";
				for classrow in GameInfo.UnitClasses( otherCondition ) do
					defaultUnit = GameInfo.Units[classrow.DefaultUnit];
				end
				if defaultUnit then
		...
		if defaultUnit then
			...
			if thisUnitInstance then
				...
				UpdateSmallButton( buttonAdded, thisUnitInstance.ReplaceImage, thisUnitInstance.ReplaceButton, textureSheet, textureOffset, CategoryUnits, Locale.ConvertTextKey( defaultUnit.Description ), defaultUnit.ID );
				buttonAdded = buttonAdded + 1;
			end
		end
