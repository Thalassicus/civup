...
function UpdateData()
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		strFaithStr = string.format("%i (+%i)", activePlayer:GetFaith(), activePlayer:GetTotalFaithPerTurn());
...
function FaithTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_FAITH_ACCUMULATED", activePlayer:GetFaith());
		strText = strText .. "[NEWLINE]";
	
		-- Faith from Cities
		local iFaithFromCities = activePlayer:GetFaithPerTurnFromCities();
		...
		local iFaithFromMinorCivs = activePlayer:GetFaithPerTurnFromMinorCivs();
		...
		-- Faith from Religion
		local iFaithFromReligion = activePlayer:GetFaithPerTurnFromReligion();
		...
		if (iFaithFromCities ~= 0 or iFaithFromMinorCivs ~= 0) then
			strText = strText .. "[NEWLINE]";
		end
