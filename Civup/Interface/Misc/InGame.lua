...
function( wParam, lParam )
	if( wParam == Keys.VK_ESCAPE ) then
        ...
        return true;
-- Cope: [ WORKEDTILES ] Listen for shift keydown as well as mouseover movement further down
    elseif ( wParam == Keys.VK_SHIFT ) then
		OnMouseOverCityPlot( UI.GetMouseOverHex() );
	end
----------
    return false;
end
...
-- Cope: [ WORKEDTILES ] Listen for mouseovers on city plots while CONTROL is pressed, will trigger
--		 the new Show Worked Tiles feature
-- Blue Skies Over Bad Lands

function OnMouseOverCityPlot( hexX, hexY )
	if UIManager:GetShift() then
		local plot = Map.GetPlot( hexX, hexY );
		
		if ( plot:IsCity() and plot:GetOwner() == Game.GetActivePlayer() ) then
			amDisplayingWorkedTiles = true;
			local city = plot:GetPlotCity();
			if( plot:GetOwner() == city:GetOwner() ) then
				local hColor = Vector4( 0.98, 0.9, 0.55, 1.0 );
				--Events.GenericWorldAnchor( GenericWorldAnchorTypes.WORLD_ANCHOR_WORKER, true, plot, city, BuildType.WORKED_TILE );
				Events.RequestYieldDisplay( YieldDisplayTypes.CITY_OWNED, city:GetX(), city:GetY() );
				for i = 0, city:GetNumCityPlots() - 1, 1 do
					local tile = city:GetCityIndexPlot( i );
					if  city:IsWorkingPlot(tile)  then
						Events.SerialEventHexHighlight( ToHexFromGrid( Vector2( tile:GetX(), tile:GetY() ) ), true, 
														hColor, "GroupBorder");
					end
				end
			end
	...
	elseif (amDisplayingWorkedTiles) then
		amDisplayingWorkedTiles = false; --This is a dirty, dirty hack, but I'm tired :) --NOCH ZU VERBESSERN!
		RequestYieldDisplay();
		Events.ClearHexHighlightStyle("GroupBorder");
	end

end
Events.SerialEventMouseOverHex.Add( OnMouseOverCityPlot );
-------------
----------------------------------------------------------------        
----------------------------------------------------------------        
local TOP    = 0;
...
for addin in Modding.GetActivatedModEntryPoints("InGameUIAddin") do
	...
	if not addinFile then
		print(string.format("Invalid addin: ModID=%s Version=%s File=%s", addin.ModID, addin.Version, addin.File));
	end
