...
--	if numUnlockedBranches > 0 then
		...
		while policyBranchInfo ~= nil do
			...
			local strTip = Locale.ConvertTextKey(policyBranchInfo.Help) or " ";
	
			-- AI Flavor Debug
			if Civup.SHOW_AI_PRIORITY_POLICIES == 1 then
				strTip = strTip .. "[NEWLINE][NEWLINE]----------------[NEWLINE]AI Priorites (debug info):"
				...
				for flavorInfo in GameInfo.Policy_Flavors(string.format("PolicyType = '%s'", policyInfo.Type)) do
					strTip = strTip .. string.format("[NEWLINE]%s %s", flavorInfo.Flavor, Locale.ToLower(string.gsub(flavorInfo.FlavorType, "FLAVOR_", "")));
				end
			end
			...
			if not player:IsPolicyBranchUnlocked( i ) then
				...
				if (not player:CanUnlockPolicyBranch(i)) then
					...
					strTip = strTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK");
					
					-- Not in prereq Era
					if (bEraLock) then
						local strEra = GameInfo.Eras[iEraPrereq].Description;
						strTip = strTip .. " " .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_ERA", strEra);
					...
					-- Don't have enough Culture Yet
						...
						strTip = strTip .. " " .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_CULTURE", player:GetYieldNeeded(YieldTypes.YIELD_CULTURE));
				...
				-- Can adopt this branch right now
					...
					strTip = strTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_UNLOCK_SPEND", player:GetYieldNeeded(YieldTypes.YIELD_CULTURE));
			...
			elseif (player:IsPolicyBranchBlocked(i)) then
				...
				strTip = strTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_BLOCKED");
			...
			-- Update tooltips
			thisButton:SetToolTipString(strTip);
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil) then
			...
			-- Tooltip
			local strTip = Locale.ConvertTextKey( policyInfo.Help ) or " ";
			...
			-- Policy is locked
				...
				IconHookup( policyInfo.PortraitIndex, 64, policyInfo.IconAtlas, thisPolicyIcon.PolicyImage );
				-- Tooltip
				strTip = strTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_CANNOT_UNLOCK");
			end
			...
			thisPolicyIcon.PolicyIcon:SetToolTipString( strTip );
		end
