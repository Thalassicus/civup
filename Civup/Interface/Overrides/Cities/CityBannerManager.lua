	...
	if city ~= nil then
		...
		if (isActiveTeamCity) then
			if city:IsCapital() or (player:IsCapitalConnectedToCity(city) and not city:IsBlockaded()) then
				...
				if city:IsHasBuilding(GameInfo.Buildings.BUILDING_TRADE_CONNECTION.ID) or string.find(GetProductionTooltip(city), Locale.ConvertTextKey("TXT_KEY_ROUTE_RAILROAD")) then
					controls.ConnectedIcon:SetString("[ICON_TRADE_WHITE]")
...
function OnSpecificCityInfoDirty(iPlayerID, iCityID, eUpdateType)
	...
	if (eUpdateType == CityUpdateTypes.CITY_UPDATE_TYPE_BANNER or
	    eUpdateType == CityUpdateTypes.CITY_UPDATE_TYPE_ENEMY_IN_RANGE) then
