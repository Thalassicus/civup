...
function UpdateScreen()
    ...
    Controls.SupplyCapValue:SetText(        GetMaxUnitSupply(pPlayer) );
    Controls.SupplyUseValue:SetText(        GetCurrentUnitSupply(pPlayer) );

    local iSupplyDeficit = math.max(0, GetCurrentUnitSupply(pPlayer) - GetMaxUnitSupply(pPlayer));
    local bInDeficit = (iSupplyDeficit ~= 0 );
    
    if( not bInDeficit ) then
		Controls.SupplyRemainingValue:SetText( GetMaxUnitSupply(pPlayer) - GetCurrentUnitSupply(pPlayer) );
    else
	    Controls.SupplyDeficitValue:SetText( iSupplyDeficit );
	    Controls.SupplyDeficitPenaltyValue:SetText( pPlayer:GetSupplyModifier(YieldTypes.YIELD_PRODUCTION, true) .. "%" );
    end
