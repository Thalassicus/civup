...
function OnTechPanelUpdated()
	...
	if (eCurrentTech ~= -1) then
		...
		local iResearchTurnsLeft = pPlayer:GetYieldTurns(YieldTypes.YIELD_SCIENCE,  eCurrentTech);
		local iCurrentResearchProgress = pPlayer:GetYieldStored(YieldTypes.YIELD_SCIENCE,  eCurrentTech);
		local iResearchNeeded = pPlayer:GetYieldNeeded(YieldTypes.YIELD_SCIENCE,  eCurrentTech);
		local iResearchPerTurn = pPlayer:GetYieldRate(YieldTypes.YIELD_SCIENCE);
		...
		if pTechInfo then
			...
			if iResearchPerTurn > 0 then
				szText = szText .. " (" .. tostring(math.ceil(iResearchTurnsLeft)) .. ")";
			end		
			local bShowProgress = true;
			local customHelpString = GetHelpTextForTech(eCurrentTech, bShowProgress);
...
function OnPopup( popupInfo )	
    if( popupInfo.Type == ButtonPopupTypes.BUTTONPOPUP_CHOOSETECH and
...
----------------------------------------------------------------
Events.GameplaySetActivePlayer.Add(OnTechPanelUpdated);
