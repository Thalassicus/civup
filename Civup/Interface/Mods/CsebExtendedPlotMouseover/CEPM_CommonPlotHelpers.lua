...
function csebPlotHelpers_GetImprovementListForResource(pPlot, pResource)
	...
	local impID = pPlot:GetRevealedImprovementType(iActiveTeam, false); -- get the improvement type on the plot
...
function csebPlotHelpers_CanBeBuilt(pPlot, pBuild)
	...
	local iResource		= pPlot:GetResourceType(iActiveTeam)
	local impID			= pPlot:GetRevealedImprovementType(iActiveTeam, false);

	if not pBuild.ImprovementType then -- if this is an improvement build
		return false;
	end
	...
	if (impID == GameInfoTypes [ pBuild.ImprovementType ]) then
		return false;
	end
