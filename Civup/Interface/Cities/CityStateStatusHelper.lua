...
function GetCityStateStatusRow(majorCivID, minorCivID)
	...
	local iInf = minorCiv:GetMinorCivFriendshipWithMajor(majorCivID);
	local isWar = majorTeam:IsAtWar(minorTeamID);
	local canBully = minorCiv:CanMajorBullyGold(majorCivID);
	local bAllies = minorCiv:IsAllies(majorCivID);
...
function GetCityStateStatusToolTip(majorCivID, minorCivID, bFullInfo)
	...
	-- Bullying
	local shortBully = canBully and "TXT_KEY_POSSIBLE" or "TXT_KEY_NOT_POSSIBLE"
	local longBully  = canBully and "TXT_KEY_CSTATE_CAN_BULLY" or "TXT_KEY_CSTATE_CANNOT_BULLY"
	strStatusTT = string.format(
		"%s[NEWLINE]%s[COLOR_POSITIVE_TEXT]%s[ENDCOLOR]%s",
		strStatusTT,
		Locale.ConvertTextKey("TXT_KEY_DIPLO_STATUS_TRIBUTE_TT"),
		Locale.ConvertTextKey(shortBully),
		showBasicHelp and ("[NEWLINE]"..Locale.ConvertTextKey(longBully)) or " "
	)
...
function GetActiveQuestToolTip(majorCivID, minorCivID)
	...
	for i, eType in ipairs(ktQuestsDisplayOrder) do
		if (minorCiv:IsMinorCivActiveQuestForPlayer(majorCivID, eType)) then
			...
			elseif (eType == MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_CULTURE) then
				local iLeaderScore = minorCiv:GetMinorCivContestValueForLeader(MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_CULTURE);
				local iMajorScore = minorCiv:GetMinorCivContestValueForPlayer(majorCivID, MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_CULTURE);
			...
			elseif (eType == MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_FAITH) then
				local iLeaderScore = minorCiv:GetMinorCivContestValueForLeader(MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_FAITH);
				local iMajorScore = minorCiv:GetMinorCivContestValueForPlayer(majorCivID, MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_FAITH);
			...
			elseif (eType == MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_TECHS) then
				local iLeaderScore = minorCiv:GetMinorCivContestValueForLeader(MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_TECHS);
				local iMajorScore = minorCiv:GetMinorCivContestValueForPlayer(majorCivID, MinorCivQuestTypes.MINOR_CIV_QUEST_CONTEST_TECHS);
	...
	if (minorCiv:IsThreateningBarbariansEventActiveForPlayer(majorCivID)) then
		...
		sToolTipText = sToolTipText .. "[ICON_BULLET]";
		sToolTipText = sToolTipText .. Locale.Lookup( "TXT_KEY_NOTIFICATION_MINOR_BARBS_QUEST", Players[minorCivID]:GetCivilizationShortDescriptionKey() );
	end
