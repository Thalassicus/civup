...
function City_GetBeliefFollowers(city, beliefType)
	...
	local beliefID = GameInfo.Beliefs[beliefType]
	...
	if player:HasCreatedReligion() then
		...
		for i, religionBeliefID in ipairs(Game.GetBeliefsInReligion(religionID)) do
			if religionBeliefID == beliefID then
				return city:GetNumFollowers(religionID)
			end
		end
	elseif player:HasCreatedPantheon() and player:GetBeliefInPantheon() == beliefID then
		return city:GetNumFollowers(GameInfo.Religions.RELIGION_PANTHEON.ID)
	end
	return 0
end
...
function City_GetBuildingsOfFlavor(city, flavorType, budget, includeWonders)
	...
	for flavorInfo in GameInfo.Building_Flavors(string.format("FlavorType = '%s'", flavorType)) do
		...
		if (City_CanBuild(city, itemInfo.ID, 0, 1)
			and (includeWonders or not Building_IsWonder(itemInfo.ID))
			and (not budget or City_GetPurchaseCost(city, YieldTypes.YIELD_GOLD, GameInfo.Buildings, itemInfo.ID) <= budget)
			)then
...
function City_GetUnitsOfFlavor(city, flavorType, budget)
	...
	for flavorInfo in GameInfo.Unit_Flavors(string.format("FlavorType = '%s'", flavorType)) do
		...
		if not itemInfo then
			--log:Warn("City_GetUnitsOfFlavor: %s does not exist in GameInfo.Units  (FlavorType=%s)", flavorInfo.UnitType, flavorType)
		elseif itemInfo.Class ~= "UNITCLASS_SCOUT" and city:CanTrain(itemInfo.ID, 0, 1) and (not budget or City_GetPurchaseCost(city, YieldTypes.YIELD_GOLD, GameInfo.Units, itemInfo.ID) <= budget) then
			local isResourceAvailable = true
