...
function Plot_Buy(plot, player, city, cost)
	...
	MapModData.VEM.PlotCostExtra[playerID] = MapModData.VEM.PlotCostExtra[playerID] + GameDefines.PLOT_ADDITIONAL_COST_PER_PLOT
	SaveValue(MapModData.VEM.PlotCostExtra[playerID], "MapModData.VEM.PlotCostExtra[%s]", playerID)
end

function Plot_GetCost(city, plot)
	return city:GetBuyPlotCost(plot:GetX(), plot:GetY()) + MapModData.VEM.PlotCostExtra[city:GetOwner()]
end

if not MapModData.VEM.PlotCostExtra then
	MapModData.VEM.PlotCostExtra = {}
	startClockTime = os.clock()
	for playerID, player in pairs(Players) do
		if UI.IsLoadedGame() then
			MapModData.VEM.PlotCostExtra[playerID] = LoadValue("MapModData.VEM.PlotCostExtra[%s]", playerID) or 0
		else
			MapModData.VEM.PlotCostExtra[playerID] = 0
		end
	end
	if UI:IsLoadedGame() then
		log:Warn("%-10s seconds loading PlotCostExtra", Game.Round(os.clock() - startClockTime, 8))
	end
end
...
--[[
if not MapModData.VEM.InitAreaWeightValues then
	MapModData.VEM.InitAreaWeightValues = true
