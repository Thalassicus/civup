...
----------------------------------------------------------------
-- Game.GetAdjustedTurn(turn)
--
function Game.GetAdjustedTurn(turn)
	return turn * GameInfo.GameSpeeds[Game.GetGameSpeedType()].VictoryDelayPercent / 100
end
...
function Building_IsWonder(buildingType)
	local buildingClassInfo = GameInfo.BuildingClasses[GameInfo.Buildings[buildingType].BuildingClass]
	return (buildingClassInfo.MaxPlayerInstances == 1) or (buildingClassInfo.MaxGlobalInstances == 1)
end
