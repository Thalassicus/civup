...
function PlayerClass.GetBuildingYield(player, city, buildingID, yieldID)
	...
	if city then
		...
		for row in GameInfo.Belief_BuildingClassYieldChanges(query) do
			...
			if City_GetBeliefFollowers(city, beliefInfo.Type) >= beliefInfo.MinFollowers  then
				yield = yield + row.YieldChange
			end
		end
...
function PlayerClass.GetBuildingYieldMod(player, city, buildingID, yieldID)
	local buildingInfo = GameInfo.Buildings[buildingID]
	local yield = 0
	local query = ""
	if yieldID == nil then
...
function City_GetBaseYieldFromBuildings(city, yieldID)
	...
	for building in GameInfo.Buildings() do
		...
		if numBuilding > 0 and (building.Type ~= "BUILDING_HAPPINESS_GLOBAL") then
			yield = yield + player:GetBuildingYield(city, building.ID, yieldID) * numBuilding
		end
	end
	return yield
end
...
function City_GetBaseYieldModFromBuildings(city, yieldID)
	...
	for building in GameInfo.Buildings() do
		...
		if numBuilding > 0 then
			yield = yield + player:GetBuildingYieldMod(city, building.ID, yieldID) * numBuilding
		end
	end
	return yield
end
...
function PlayerClass.GetYieldRate(player, yieldID, skipGlobalMods)
	...
	elseif yieldID == YieldTypes.YIELD_HAPPINESS then
		...
		if not player:IsMinorCiv() then
			...
			yield = yield - City_GetNumBuilding(player:GetCapitalCity(), GameInfo.Buildings.BUILDING_HAPPINESS_GLOBAL.ID)
		end
		return yield
...
function PlayerClass.UpdateModdedHappiness(player)	
	...
	--log:Debug("%5s %20s global happiness from mod", yield, player:GetName())
	capital:SetNumRealBuilding(GameInfo.Buildings.BUILDING_HAPPINESS_GLOBAL.ID, yield)
	...
	for city in player:Cities() do
		...
		log:Debug("%5s %20s local happiness", yield, city:GetName())
		city:SetNumRealBuilding(GameInfo.Buildings.BUILDING_HAPPINESS_LOCAL.ID, yield)
	end
	--]]
end
