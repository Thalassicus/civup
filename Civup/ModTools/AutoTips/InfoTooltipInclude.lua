...
GetDefaultBuildingFieldData = GetDefaultBuildingFieldData or function (iBuildingID, fieldType, bExcludeName, bExcludeHeader, bNoMaintenance)
	...
	local yieldMod = (
		...
		or (fieldType == "FaithMod" and "YIELD_CULTURE")
	)
	...
	elseif fieldType == "UnmoddedHappiness" then
		return Player_GetBuildingYield(activePlayer, iBuildingID, YieldTypes.YIELD_HAPPINESS)
		
	elseif fieldType == "Happiness" then
		return buildingInfo.Happiness

	elseif yieldChange then
		return Player_GetBuildingYield(activePlayer, iBuildingID, YieldTypes[yieldChange])

	elseif yieldMod then
		return Player_GetBuildingYieldMod(activePlayer, iBuildingID, YieldTypes[yieldMod])
...
function GetCityStateStatus(pPlayer, iForPlayer, bWar)
	...
	if rival then	
		...
		strStatusTT = strStatusTT .. "[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_DIPLO_STATUS_OTHER_TT", rival, rivalInfluence);
	end
...
function GetFaithTooltip(city)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		local iFaithFromBuildings = city:GetFaithPerTurnFromBuildings();
		...
		-- Faith from Traits
		local iFaithFromTraits = city:GetFaithPerTurnFromTraits();
		...
		-- Faith from Terrain
		local iFaithFromTerrain = city:GetBaseYieldRateFromTerrain(YieldTypes.YIELD_FAITH);
		...
		-- Faith from Policies
		local iFaithFromPolicies = city:GetFaithPerTurnFromPolicies();
		...
		-- Faith from Religion
		local iFaithFromReligion = city:GetFaithPerTurnFromReligion();
		...
		table.insert(faithTips, GetReligionTooltip(city));
	end
...
function GetYieldTooltip(city, yieldID)
	...
	local iYieldFromReligion = city:GetBaseYieldRateFromReligion(yieldID);
