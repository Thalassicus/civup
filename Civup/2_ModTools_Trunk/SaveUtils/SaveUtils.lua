...
-----------------------------------------------------------------------------
  ...
  save( player, key1, "this" );				--ONLY string and integer keys recommended.
  save( player, key2, "200" );				--loads as string.
  save( player, key3, 300 );				--loads as number.
  save( player, key4, true );				--loads as boolean.
  save( player, nil,  "that" );				--automatic integer index.
  save( player, key1, table, "otherMod" );	--optional fourth parameter.
  save( pPlot,   ...,  ...,   ... );		--same as above.
  save( pUnit,   ...,  ...,   ... );		--same as above.

  delete( player, key1 );					--deletes key and value from table.
  delete( player, key1, "otherMod" );		--optional third parameter.

  data = load( player, key1 );
  data = load( player, key1, "otherMod" );	--optional third parameter.
  data = load( pPlot,   ...,  ... );		--same as above.
  data = load( pUnit,   ...,  ... );		--same as above.

  data = load( player );					--entire mod table for player.
  data = load( player, nil, "otherMod" );	--entire mod table for player.

  data = load( player, nil, false );		--entire save table for player.
...
using a table for reference, ie: load(player,table), will produce
...
following will always work as expected.
  ...
  save(player,{"table key"},"value");
  for k,v in pairs(load(player)) do
...
0 = no cache, 1 = serialize on save(), 2 = serialize on sync().  Default: 1.
  ...
  clear( player ); --clears player cache.

  sync();           --error.
  sync( true );     --serializes entire cache.
  sync( player );  --serializes player cache.
...
table, ie: load(player, nil, false, 0), because logically mod false can not
...
Other:
  ...
  WARN_NOT_SHARED = false; include("SaveUtils");
  MY_MOD_NAME = "MyMod";
]]
--===========================================================================
-- Save Class
...
function setCacheState( state, mod )
  ...
  if not t == "number" then
    --log:Fatal( "setCacheState(): Invalid first argument of type "..t..", expected number." );
    return false; --error.
  end
  if mod == nil then mod = MY_MOD_NAME; end
  t = type( mod );
  if not (t == "number" or t == "string") then
    --log:Fatal( "setCacheState(): Invalid second argument of type "..t..", expected number, or string." );
    return false; --error.
  end
...
function getCacheState( mod )
  ...
  local t = type( mod );
  if not (t == "number" or t == "string") then
    --log:Fatal( "getCacheState(): Invalid first argument of type "..t..", expected number, or string." );
    return false; --error.
  end
...
function clear( target )
  ...
  if not (t == "table" or target == true) then
    --log:Fatal( "clear(): Invalid first argument of type "..t..", expected table, or boolean true." );
    return false; --error.
  end
  if WARN_NOT_SHARED ~= false and
      not hasShared( "SaveUtils.g_pCache" ) then
    --log:Warn( "Warning: cache not shared." ); --warning.
  end
  if target == true then
...
function sync( target )
  local t = type( target );
  if not (t == "table" or target == true) then
    --log:Fatal( "sync(): Invalid first argument of type "..t..", expected table, or boolean true." );
    return false; --error.
  end
  if WARN_NOT_SHARED ~= false and
      not hasShared( "SaveUtils.g_pCache" ) then
    --log:Warn( "cache not shared." ); --warning.
  end
  if target == true then
    ...
    if pSave == nil then
      --log:Fatal( "sync(): Target not found." ); return false; --error.
...
function save( target, key, value, mod, cacheState )
  local t = type( target );
  if not t == "table" then
    --log:Fatal( "save(): Invalid first argument of type "..t..", expected table." );
    return false; --error.
  end  
  if mod == nil then mod = MY_MOD_NAME; end
  t = type( mod );
  if not (t == "number" or t == "string") then
    --log:Fatal( "save(): Invalid fourth argument of type "..t..", expected number, or string." );
    return false; --error.
  end
  t = type( cacheState );
  if not (cacheState == nil or t == "number") then
    --log:Fatal( "save(): Invalid fifth argument of type "..t..", expected nil, or number." );
    return false; --error.
  end
  local pSave = nil;
...
function load( target, key, mod, cacheState )
  local t = type( target );
  if not t == "table" then
    --log:Fatal( "load(): Invalid first argument of type "..t..", expected table." );
    return nil; --error.
  end    
  if mod == nil then mod = MY_MOD_NAME; end
  t = type( mod );
  if not (t == "number" or t == "string" or mod == false) then
    --log:Fatal( "load(): Invalid third argument of type "..t..", expected number, string, or boolean false." );
    return nil; --error.
  end
  if mod == false then mod = nil; end
  t = type( cacheState );
  if not (cacheState == nil or t == "number") then
    --log:Fatal( "load(): Invalid fourth argument of type "..t..", expected nil, or number." );
    return nil; --error.
  end
  local pSave = nil;
  local iCache = DEFAULT_CACHE_STATE;
  ...
  elseif iCache > 0 then
    if WARN_NOT_SHARED ~= false then
      ...
      if not warned and not hasShared( "SaveUtils.g_cacheState" ) then
        --log:Warn( "cache-state not shared." ); --warning.
      end
    end
...
function serialize( p )
  ...
  if t == "function" or t == "userdata" or t == "thread" then
    --log:Fatal( "serialize(): Invalid type: "..t ); --error.
  elseif p ~= nil then
    if t ~= "table" then
...
function share( key, data, context )
  ...
  if not (t == "number" or (t == "string" and key ~= "")) then
    --log:Fatal( "share(): Invalid first argument of type "..t..", expected number, or unempty-string." );
    return data; --error.
  end
  local t = type( context );
  if not (t == "nil" or (t == "string" and context ~= "")) then
    --log:Fatal( "share(): Invalid third argument of type "..t..", expected nil, or unempty-string." );
    return data; --error.
  end
...
function setShared( key, data, context )
  if key == nil and context == nil then
    --log:Fatal( "setShared(): Invalid first and third argument combination of ".."types "..type( key ).." and "..type( context )..", expected any type except nil for key when context nil." );
    return nil; --error.
  end
