...
function addCustomNotification(noty)
	if not lowestFreeID then
		log:Error("addCustomNotification: lowestFreeID=nil for %s", noty.Name)
	end
...
function OnCustomNotificationsLoaded(controls, processStackSizes, lowestfreeid)
	...
	log:Info("OnCustomNotificationsLoaded controls=%s processStackSizes=%s lowestfreeid=%s", Controls, ProcessStackSizes, lowestFreeID)
end
LuaEvents.CustomNotificationLoaded.Add(OnCustomNotificationsLoaded)
--aprint("Custom notification executed")

print(string.format("%3s ms loading CustomNotification.lua", Game.Round((os.clock() - startClockTime)*1000)))