...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	local theirPlayerID = pCity:GetOwner();
	...
	if iMyStrength <= 0 then
		...
		local sizeY = Game.Constrain(minRows, math.max(myRows, theirRows), maxRows) * rowWidth + rowBuffer;
		----log:Debug("myRows=%s constrained=%s width=%s buffer=%s", myRows, Game.Constrain(minRows, math.max(myRows, theirRows), maxRows), Game.Constrain(minRows, math.max(myRows, theirRows), maxRows) * rowWidth, rowBuffer)
		Controls.DetailsGrid:DoAutoSize();
		Controls.DetailsGrid:SetSizeX( sizeX );
		Controls.DetailsGrid:SetSizeY( sizeY );
		Controls.DetailsSeperator:SetSizeY( sizeY );
		Controls.DetailsGrid:ReprocessAnchoring();
		--Controls.HealthBars:SetOffsetVal( 0, Controls.HealthBars:GetSizeY() + healthOffset - sizeY );
		return
	end
	...
	if (iModifier ~= 0 and theirPlayer:IsMinorCiv()) then
		controlTable = g_MyCombatDataIM:GetInstance();
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	local iMyPlayer = pMyUnit:GetOwner();
	local theirPlayerID = pTheirUnit:GetOwner();
	local pMyPlayer = Players[iMyPlayer];
	local theirPlayer = Players[theirPlayerID];
	...
	if not pTheirUnit:IsCombatUnit() then
		-------------------------
		-- PRIZE SHIPS PREVIEW --
		-------------------------
		if (not bRanged and pMyUnit.GetCaptureChance ~= nil) then
			local iChance;
...
--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
	...
	-- Bonus for fighting in one's lands
	if (pToPlot:IsFriendlyTerritory(theirPlayerID)) then
		...
		iModifier = theirPlayer:GetFoundedReligionFriendlyCityCombatMod(pToPlot);
		if (iModifier ~= 0) then
			controlTable = g_TheirCombatDataIM:GetInstance();
			controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_FRIENDLY_CITY_BELIEF_BONUS" );
			controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
	...
	-- Bonus for fighting outside one's lands
	if (not pToPlot:IsFriendlyTerritory(theirPlayerID)) then
		...
		iModifier = theirPlayer:GetFoundedReligionEnemyCityCombatMod(pToPlot);
		if (iModifier ~= 0) then
			controlTable = g_TheirCombatDataIM:GetInstance();
			controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ENEMY_CITY_BELIEF_BONUS" );
			controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
...
----				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_MOD_VS_TYPE", iModifier, GC.GetUnitClassInfo(pMyUnit:GetUnitClassType()).GetTextKey()));
	...
	-- CombatBonusVsHigherTech
	if (pToPlot:GetOwner() == theirPlayerID) then
...
--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
		...
		-- Bonus for fighting in one's lands
		if (theirPlot:IsFriendlyTerritory(theirPlayerID)) then
			...
			iModifier = theirPlayer:GetFoundedReligionFriendlyCityCombatMod(theirPlot);
			if (iModifier ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_FRIENDLY_CITY_BELIEF_BONUS" );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
		...
		-- Bonus for fighting outside one's lands
		if (not theirPlot:IsFriendlyTerritory(theirPlayerID)) then
			...
			iModifier = theirPlayer:GetFoundedReligionEnemyCityCombatMod(theirPlot);
			if (iModifier ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ENEMY_CITY_BELIEF_BONUS" );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
...
function DoUpdateHealthBars(iMaxMyHP, iTheirMaxHP, myCurrentDamage, theirCurrentDamage, iMyDamageInflicted, iTheirDamageInflicted)
	...
	local healthBarSize = { x = 8, y = math.floor(fullSize * healthPercent) };
	...
	if myDamageTaken > 0 then
		...
		local damageBarSize = { x = 8, y = math.floor(fullSize * damagePercent) };
	...
	healthBarSize = { x = 8, y = math.floor(fullSize * healthPercent) };
	if healthTimes100 <= 30 then
