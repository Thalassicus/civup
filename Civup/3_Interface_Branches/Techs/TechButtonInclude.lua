...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize )
	...
	-- add the stuff granted by this tech here
 	...
 	for thisBuildingInfo in GameInfo.Buildings(string.format("PreReqTech = '%s'", techType)) do
		...
		if validBuildingBuilds[thisBuildingInfo.BuildingClass] == thisBuildingInfo.Type then
			...
			if thisBuildingInfo.IsVisible and thisBuildingInfo.IsMarketplace == 0 and not Game.HasValue({BuildingType=thisBuildingInfo.Type}, GameInfo.Building_Addons) then
				local buttonName = "B"..tostring(buttonNum);
				local thisButton = thisTechButtonInstance[buttonName];
				if thisButton then
	...
	for row in GameInfo.Technology_DomainPromotion(condition) do
		...
		if thisButton then
			...
			thisButton:SetToolTipString(
				Locale.ConvertTextKey("TXT_KEY_TECH_PROMOTION_DOMAIN",
				GameInfo.Domains[row.DomainType].Description,
				GameInfo.UnitPromotions[row.PromotionType].Help
				)
			);
			buttonNum = buttonNum + 1;
		else
	...
	if ( tech.UnitFortificationModifier and tech.UnitFortificationModifier > 0 ) then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
	...
	if ( tech.UnitBaseHealModifier and tech.UnitBaseHealModifier > 0 ) then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
	...
	if GameInfo.Technology_FreePromotions then
...
function AdjustArtOnGrantedBuildingButton( thisButton, thisBuildingInfo, textureSize )
	...
	if thisButton then
		...
		thisButton:SetTextureOffset( textureOffset );
		thisButton:SetHide( false );
		if Building_IsAlreadyBuilt(thisBuildingInfo.ID, Players[Game.GetActivePlayer()]) then
