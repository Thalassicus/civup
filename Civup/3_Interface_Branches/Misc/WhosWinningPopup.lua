		...
		for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
			...
			if (pLoopPlayer:IsAlive()) then
				if (pLoopPlayer:GetNumPolicies() < 2) then
					--print("Progress Screen: Can't display Culture Mode because at least 1 pPlayer has fewer than 2 Policies!");
					return false;
				end
			end
		end
...
function GetScoreFromMode(iPlayer, iMode)
	...
	if (iMode == g_ListMode_Food) then
		...
		for pLoopCity in pPlayer:Cities() do
			iTotalFoodSurplus = iTotalFoodSurplus + City_GetYieldRate(pLoopCity, YieldTypes.YIELD_FOOD);
	...
	elseif (iMode == g_ListMode_Production) then
		...
		for pLoopCity in pPlayer:Cities() do
			iTotalProduction = iTotalProduction + City_GetYieldRate(pLoopCity, YieldTypes.YIELD_PRODUCTION);
			if pLoopCity:IsFoodProduction() then
				iTotalProduction = iTotalProduction - City_GetYieldRate(pLoopCity, YieldTypes.YIELD_FOOD);
			end
			iTotalCities = iTotalCities + 1;
		end
