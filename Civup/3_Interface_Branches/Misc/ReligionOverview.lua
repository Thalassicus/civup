...
function RefreshYourReligion()
	...
	if (iYieldFromCities ~= 0) then
		...
		entry.FaithModifier:LocalizeAndSetText("TXT_KEY_TP_FAITH_FROM_CITIES", iYieldFromCities);
	end

	-- Faith from Minor Civs
	local iYieldFromMinorCivs = (Civup.ENABLE_DISTRIBUTED_MINOR_CIV_YIELDS == 1) and player:GetFinalCitystateYield(yieldID) or player:GetFaithPerTurnFromMinorCivs()
	if (iYieldFromMinorCivs ~= 0) then
		local entry = g_FaithModifierManager:GetInstance();
		entry.FaithModifier:LocalizeAndSetText("TXT_KEY_TP_FAITH_FROM_MINORS", Game.Round(iYieldFromMinorCivs, 1));
	end
	
	-- Faith from Religion
	local iYieldFromReligion = player:GetFaithPerTurnFromReligion();
	if (iYieldFromReligion ~= 0) then
		local entry = g_FaithModifierManager:GetInstance();
		entry.FaithModifier:LocalizeAndSetText("TXT_KEY_TP_FAITH_FROM_RELIGION", iYieldFromReligion);
	end
	
	-- Possible Great People
	local possibleGreatPeople = {}
	
	function AddGreatPersonUnit(Type, Condition)
		local unit = GameInfo.Units[Type];
		if(unit ~= nil) then
			local unitDescription = unit.Description;
			possibleGreatPeople[unit.ID] = function()
				if(Condition()) then
					return Locale.Lookup(unitDescription);
				end
			end
		end	
	end
	
	AddGreatPersonUnit("UNIT_PROPHET", function()
		return (player:HasCreatedReligion());
	end);
	
	AddGreatPersonUnit("UNIT_MERCHANT", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_COMMERCE"];
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	AddGreatPersonUnit("UNIT_SCIENTIST", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_RATIONALISM"];
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	
	AddGreatPersonUnit("UNIT_ARTIST", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_FREEDOM"];
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	
	AddGreatPersonUnit("UNIT_GREAT_GENERAL", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_AUTOCRACY"];
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	
	AddGreatPersonUnit("UNIT_GREAT_ADMIRAL", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_AUTOCRACY"];
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	
	AddGreatPersonUnit("UNIT_ENGINEER", function()
		local policy = GameInfo.PolicyBranchTypes["POLICY_BRANCH_ORDER"]
		return (policy ~= nil and player:IsPolicyBranchUnlocked(policy.ID) and not player:IsPolicyBranchBlocked(policy.ID));
	end);
	
	local sortedGreatPeople  = {};
		
	for info in GameInfo.Units{Special = "SPECIALUNIT_PEOPLE"} do
		local gp = possibleGreatPeople[info.ID];
		if(gp ~= nil) then
			local desc = gp();
			if(desc ~= nil) then
				table.insert(sortedGreatPeople, "[ICON_BULLET]" .. desc);
			end
		end
	end
	...
	for i,v in ipairs(sortedGreatPeople) do
		local c = Controls["GreatPersonName" .. i];
		if(c ~= nil) then
			c:SetText(v);
			c:SetHide(false);
	...
	if(#sortedGreatPeople == 0) then
		local c = Controls["GreatPersonName" .. 1];
		c:LocalizeAndSetText("TXT_KEY_RO_YR_NO_GREAT_PEOPLE");
		c:SetHide(false);
	end
	
	Controls.GreatPersonListLeft:CalculateSize();
	Controls.GreatPersonListRight:CalculateSize();
	...
	IconHookup(civ.PortraitIndex, 80, civ.IconAtlas, Controls.FounderCivIcon);
	...
	if(capital ~= nil) then
		for unit in GameInfo.Units() do
			...
			if(faithCost > 0 and player:IsCanPurchaseAnyCity(false, true, unit.ID, -1, YieldTypes.YIELD_FAITH)) then
			    if (player.DoesUnitPassFaithPurchaseCheck == nil or player:DoesUnitPassFaithPurchaseCheck(unit.ID)) then
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (pPlayer:IsAlive()) then
			if (pPlayer:HasCreatedReligion()) then
				...
				local civPortraitIndex = civ.PortraitIndex;
				local civIconAtlas = civ.IconAtlas;
				...
				table.insert(religions, {
					...
					FounderIconIndex = civPortraitIndex,
					FounderIconAtlas = civIconAtlas,
