...
function UpdateDisplay()
    ...
    for city in pPlayer:Cities() do
    	...
    	local pct = 1 - (city:GetDamage() / city:GetMaxHitPoints());
    	if( pct ~= 1 ) then
		...
		if (instance.GrowthBar) then
			...
			local iFoodPerTurn = city:IsFoodProduction() and 0 or Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_FOOD));
		...
		sortEntry.Food = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_FOOD));
        instance.Food:SetText( sortEntry.Food );
       
		sortEntry.Production = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_PRODUCTION));
        instance.Production:SetText( sortEntry.Production );
        
        
        if(Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
			if instance.ScienceBox then
				instance.ScienceBox:SetHide(true);
			end
        else
			sortEntry.Science = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_SCIENCE));
        ...
        sortEntry.Gold = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_GOLD));
        instance.Gold:SetText( sortEntry.Gold );
        
        sortEntry.Culture = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_CULTURE));
        instance.Culture:SetText( sortEntry.Culture );
        
		if ( sortEntry.Faith and instance.Faith ~= nil and instance.FaithBox ~= nil ) then
			if(Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
				instance.FaithBox:SetHide(true);
			else
				sortEntry.Faith = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_FAITH));
...
function UpdateGPT()
    ...
    for city in pPlayer:Cities() do
        ...
        local CityIncome = Game.Round(City_GetYieldRate(city, YieldTypes.YIELD_GOLD));
    ...
    Controls.TradeIncomeValue:SetText( Locale.ToNumber( pPlayer:GetYieldFromConnectedCities(YieldTypes.YIELD_GOLD), "#.##" ) );
    ...
    for city in pPlayer:Cities() do
        ...
        if( pPlayer:IsCapitalConnectedToCity( city ) ) then
            local tradeIncome = pPlayer:GetRouteGoldTimes100( city ) / 100;
            ...
            if( tradeIncome > 0 ) then
                ...
                local strPopInfo = " (" .. city:GetPopulation() .. ")"; 
...
if ( Controls.SortFaith and Controls.ReligionIncome ~= nil ) then
	if(Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
